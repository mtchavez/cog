...
+ (void)initialize {
	...
	NSValueTransformer *statusImageTransformer = (NSValueTransformer *)[[[StatusImageTransformer alloc] init] autorelease];
    [NSValueTransformer setValueTransformer:statusImageTransformer
                                    forName:@"StatusImageTransformer"];
		NSValueTransformer *toggleQueueTitleTransformer = (NSValueTransformer *)[[[ToggleQueueTitleTransformer alloc] init] autorelease];
		...
		if ([self repeat] == RepeatAlbum)
		{
			...
			if ((i > [[self arrangedObjects] count]-1) || ([[next album] caseInsensitiveCompare:[pe album]]) || ([next album] == nil))
			{
				...
				if ([pe album] == nil)
					...
					i = [(NSNumber *)[(NSXMLNode *)[filtered objectAtIndex:0] index] intValue];
			}
